...
* The `closed_by` and `milestone` output fields on `get issue`, `create issue`, `update issue`, and `list issues` are now nested objects instead of a plain string; playbooks referencing the old flat datapath must be updated to use the new sub-fields (e.g. `closed_by.login`)
